...
DELIMITER $$
CREATE TRIGGER update_warehouse_insert_release_trigger
    AFTER UPDATE
    ON MGT_ORDERS
    FOR EACH ROW
BEGIN
...
CREATE TRIGGER orders_insert_trigger
    AFTER INSERT
    ON MGT_ORDERS_PRODUCTS_RELATIONSHIP
    FOR EACH ROW
BEGIN
...
DELIMITER //
CREATE TRIGGER warehouse_insert_release_update_trigger2
    AFTER UPDATE
    ON WAREHOUSE_INSERT_RELEASE
    FOR EACH ROW
BEGIN
